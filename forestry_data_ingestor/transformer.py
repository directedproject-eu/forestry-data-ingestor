...
def clean_and_simplify_forestry_data(
    df: pd.DataFrame, old_cols: list[str], new_cols: list[str]
) -> pd.DataFrame:
...
def append_and_save_json(new_data_list: list, file_name: str):
    ...
    if os.path.exists(file_name):
        try:
            ...
            logger.warning(
                f"Warning: {file_name} was empty or corrupted. Starting fresh."
            )
            existing_data = []
    ...
    logger.info(
        f"Updated {file_name}: Added {added_count} new records. (Total: {len(existing_data)})"
    )
